...
const ProfilePage = () => {
	...
	const {
		...
		queryFn: async () => {
			try {
				const res = await fetch(`/api/twitter-clone/user/get-profile/${username}`);
				const data = await res.json();
				console.log(data,' at the profile page')
				if (!res.ok) {
	...
	return (
		<>
			<div className='flex-[4_4_0]  border-r border-gray-700 min-h-screen '>
				...
				{!isLoading && !isRefetching && user?.user && <p className='text-center text-lg mt-4'>User not found</p>}
				<div className='flex flex-col'>
					{!isLoading && !isRefetching && user?.user && (
						<>
							...
							<div className='flex flex-col gap-4 mt-14 px-4'>
									...
									{user?.user?.link && (
										<div className='flex gap-1 items-center '>
											<>
												...
												<a
													href='https://youtube.com/Ethio_success'
													target='_blank'
													rel='noreferrer'
								...
								<div className='flex gap-2'>
									...
									<div className='flex gap-1 items-center'>
										<span className='font-bold text-xs'>{user?.user?.follower.length}</span>
